...
--
-- Host: localhost:3306
-- Generation Time: Jun 22, 2026 at 12:59 AM
...
CREATE TABLE `t_pengadaan` (
  ...
  `status_bayar` enum('Pending','Lunas') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'Pending',
  `status_kontrak` enum('Mencari Petani','Disetujui','Dibatalkan') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'Mencari Petani',
...
ALTER TABLE `t_pengadaan_detail`
  ...
  ADD KEY `fk_t_detail_pengadaan` (`id_pengadaan`),
...
--
-- Constraints for table `t_pengadaan_detail`
--
ALTER TABLE `t_pengadaan_detail`
  ADD CONSTRAINT `fk_t_detail_komoditas` FOREIGN KEY (`id_komoditas`) REFERENCES `komoditas_pangan` (`id_komoditas`) ON DELETE RESTRICT,
  ADD CONSTRAINT `fk_t_detail_pengadaan` FOREIGN KEY (`id_pengadaan`) REFERENCES `t_pengadaan` (`id_pengadaan`) ON DELETE CASCADE;
COMMIT; 
