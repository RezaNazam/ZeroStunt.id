<?php
$pageTitle = 'Tambah Data Anak';
$pageSubtitle = 'Daftarkan data balita untuk mulai memantau tumbuh kembang.';
...
<?php if (isset($_SESSION['success'])): ?>
    <div class="mb-5 rounded-2xl border border-green-100 bg-green-50 text-green-700 px-4 py-3 text-sm font-medium">
        <?= htmlspecialchars($_SESSION['success']) ?>
    </div>
    <?php unset($_SESSION['success']); ?>
<?php endif; ?>
<?php if (isset($_SESSION['error'])): ?>
    <div class="mb-5 rounded-2xl border border-red-100 bg-red-50 text-red-700 px-4 py-3 text-sm font-medium">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        ...
        <form action="/master/anak/store" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="NIK_anak" inputmode="numeric" pattern="[0-9]{16}" minlength="16" maxlength="16" required
                    placeholder="Masukkan 16 digit NIK"
                    class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none text-sm transition">
            </div>

            <!-- Nama Anak -->
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Nama Lengkap Anak</label>
                <input type="text" name="nama_anak" required placeholder="Nama lengkap balita"
                    class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none text-sm transition">
            </div>
            ...
            <!-- Simpan Button -->
            <button type="submit"
                class="w-full rounded-2xl bg-teal-600 hover:bg-teal-700 px-5 py-3 font-bold text-white transition duration-200 mt-4">
                Simpan Data Anak
            </button>
